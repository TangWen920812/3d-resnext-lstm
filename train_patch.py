...
import numpy as np
from dataloader import CustomIter_sufu2
from sklearn.metrics import roc_auc_score, accuracy_score
import csv

net_depth = 34
n_classes = 2
ctx = [mx.gpu(i) for i in [3]]
pretrain = False
resume = False
...
train_iter = CustomIter_sufu2(root=root, batch_size=batch_size, class_num=n_classes, istrain=True, shuffle=True)
test_iter = CustomIter_sufu2(root=root, batch_size=batch_size, class_num=n_classes, istrain=False, shuffle=False)

if resume:
    start_e = 0
    symbol, arg_params, aux_params = mx.model.load_checkpoint('/media/tangwen/data/4D-classification/model/resnext34-pretrain',
                                                              start_e)
...
def test_epoch(classifier, test_iter):
    ...
    while True:
        ...
        mean_acc.append(acc)
        mean_loss.append(loss.asnumpy()[0])
...
for e in range(start_e + 1, epochs):
    ...
    if auc > auc_best:
        ...
        # classifier.save_checkpoint('/media/tangwen/data/4D-classification/model/resnext34', e)
...
print(auc_best)
np.save('/media/tangwen/data/4D-classification/model/epoch_patch_seg_2modal.npy', epoch_info)
