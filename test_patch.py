...
classifier.bind(data_shapes=[['data', (batch_size, 1, img_size, img_size, img_size)]],
...
if resume:
    start_e = 36
    symbol, arg_params, aux_params = mx.model.load_checkpoint('/media/tangwen/data/4D-classification/model/resnext34', start_e)
...
def plot_auc(fpr, tpr, roc_auc, i):
    ...
    plt.savefig('./result/' + 'result_seg.png')
...
auc_score = roc_auc_score(label_all[0:45], predict_all[0:45])
fpr, tpr, thresholds = roc_curve(label_all[0:45], predict_all[0:45])
roc_auc = auc(fpr, tpr)
plot_auc(fpr, tpr, roc_auc, 4)
# acc = accuracy_score(label_all[0:45], predict_all[0:45])
print('auc: ', auc_score)

import csv
with open('./result/result_seg.csv', 'w') as file:
