...
if resume:
    start_e = 51
    symbol, arg_params, aux_params = mx.model.load_checkpoint('/media/tangwen/data/4D-classification/model/resnext34_m', start_e)
...
def plot_auc(fpr, tpr, roc_auc, i):
    ...
    plt.savefig('./result/' + 'result_m_seg.png')
...
roc_auc = auc(fpr, tpr)
plot_auc(fpr, tpr, roc_auc, 3)
# acc = accuracy_score(label_all[0:45], predict_all[0:45])
print('auc: ', auc_score)

import csv
with open('./result/result_m_seg.csv', 'w') as file:
