...
if resume:
    start_e = 0
    symbol, arg_params, aux_params = mx.model.load_checkpoint('/media/tangwen/data/4D-classification/model/resnext34_rnn-pretrain',
                                                              start_e)
...
# mx.viz.plot_network(symbol,title='resnet_rnn',save_format='pdf',hide_weights=True,
#                     shape={'data':(24,2,64,64,64), 'label':(24,1)}).view()
...
def test_epoch(classifier, test_iter):
    ...
    print('auc: ', auc)
    return auc, sum(mean_loss) / len(mean_loss)
...
for e in range(start_e + 1, epochs):
    ...
    classifier.save_checkpoint('/media/tangwen/data/4D-classification/model/resnext34_rnn', e)
    _, arg_params, aux_params = mx.model.load_checkpoint('/media/tangwen/data/4D-classification/model/resnext34_rnn', e)
    classifier.set_params(arg_params, aux_params)
    auc, test_loss = test_epoch(classifier, test_iter)
    if auc > auc_best:
        auc_best = auc
        print('best epoch on: ', e)
        os.remove('/media/tangwen/data/4D-classification/model/resnext34_rnn' + '-%04d.params' % e)
    else:
        os.remove('/media/tangwen/data/4D-classification/model/resnext34_rnn' + '-%04d.params' % e)

    epoch_info.append([e, sum(mean_loss) / len(mean_loss), train_auc, test_loss, auc])
    mean_loss, mean_acc = [], []
    predict_all, label_all = [], []
    batch_mean = [0, 0]

print(auc_best)
np.save('/media/tangwen/data/4D-classification/model/epoch_rnn_det.npy', epoch_info)
